...
class AtomlawSearchResultView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if searchword:
            ...
            filter(Q(keywords__icontains=searchword)|Q(keywords__icontains=searchword1)|Q(title__icontains=searchword)|Q(title__icontains=searchword1)|Q(answer__icontains=searchword)|Q(answer__icontains=searchword1)).order_by('pk')
            context['objects'] = objects
            context['form'] = SearchForm(initial={'searchword': searchword})
        return context
